...
## Q1: lineWidth 到底是多少？（已终局定 88）

> **拍板（2026-08-20 终局）：88**（dogfood 实证驱动，取代 2026-08-19 的暂定 78）。
> 依据：真实脚本最高频句式 `cast(sum(x) / 100 as decimal(16, 2)) as xxx` 落在
> 79~84 列，78 会把整族炸成 3 行；88 让其保持单行。case/05、08、09、10、11、12
> 六组语料已按 88 重生成，spark/17-1 的 81 字符 `in` 列表随之回归单行。
> （注：同日项目转向二开 sql-formatter 路线，自研引擎冻结为研究成果，见
> apps/vscode/README.md；88 同样作为二开的 expressionWidth。）
...
## Q7: `when C then R` 放不下时，断行点在 then 前还是结果内部？

> **拍板（2026-08-19）：then 前断行**（dogfood 反馈驱动，与两份语料的原形态相反，
> case/08、10、11 语料随之重生成；case/10 的 81 字符超宽容忍行顺带消除）。

dogfood 首日反馈：条件放得下时 `then substr(` 挂行尾、断行深埋进函数参数，
`then` 淹没在长行中间、闭括号反而与 when 对齐，可读性差。新规则把 arm 统一为：
`then` 要么与 when 同行（整个 arm 放平），要么独行与 when 对齐（结果挂 then 上）。
- A. then 前断行（已选）：when 行放条件（须放平），then 独行对齐 when
- B. 维持语料原状：then 挂行尾，断行进结果内部，短 arm 容忍 81 字符超宽
